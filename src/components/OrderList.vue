<template>
  <div>
    ...
    <div v-for="order in orders" :key="order.id" class="bg-white p-3 mb-2 border-bottom position-relative">
      <router-link :to="isAdmin ? `/admin/pesanan/${order.id}` : `/pesanan/${order.id}`" class="text-decoration-none text-dark d-block">
